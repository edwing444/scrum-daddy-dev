...
@app.route("/wfhRequests/checkPending", methods=["GET"])
def auto_reject_wfh_requests():
    pending_requests = db.session.query(WFHRequest).filter(
        WFHRequest.overall_status == Status.PENDING
    ).all()
    ...
    for request in pending_requests:
        ...
        for entry in entries:
            ...
            if is_one_working_day_before(entry_date):
                entry.status = Status.AUTO_REJECTED
        ...
        all_entries = WFHRequestEntry.query.filter_by(request_id=request.request_id).all()
        status_set = set(entry.status for entry in all_entries)

        if len(status_set) > 1:
            overall_status = Status.REVIEWED
        else:
            overall_status = Status.AUTO_REJECTED
        
        notification_status = NotificationStatus.AUTO_REJECTED

        request_record = WFHRequest.query.get(request.request_id)
        if request_record:
            if request_record.overall_status != overall_status:
                audit_trail = AuditTrail(
                    request_id=request.request_id,
    ...
    return jsonify({
        "status_code": 200,
        "message": "Pending WFH requests have been auto-rejected where applicable."
    }), 200

def is_one_working_day_before(entry_date):
    singapore_tz = pytz.timezone('Asia/Singapore')
    current_date = datetime.now(singapore_tz)
    
    one_working_day_before = entry_date
    business_days = 0
    while business_days < 1:
        one_working_day_before -= timedelta(days=1)
        if one_working_day_before.weekday() < 5:  # Mon-Fri are weekdays
            business_days += 1
    
    
    if entry_date.weekday() == 0:  # Monday
        last_working_day = one_working_day_before  # This will be Friday in this case
        return current_date.date() >= last_working_day.date()
    else:
        return current_date.date() == one_working_day_before.date()


@app.route("/wfhRequests/getWeeklyCount", methods=["GET"])
def get_weekly_wfh_count():

    entry_date = request.args.get('entry_date')
    staff_id = int(request.args.get('staff_id'))

    entry_date = datetime.strptime(entry_date, "%Y-%m-%d").date()
    start_of_week = entry_date - timedelta(days=entry_date.weekday())  # Monday
    
    week_dates = [(start_of_week + timedelta(days=i)) for i in range(5)]  # Excludes weekends

    print("Dates: ", week_dates)

    requests = db.session.query(WFHRequest).filter(
        WFHRequest.requester_id == staff_id,
        WFHRequest.overall_status.in_([Status.APPROVED, Status.REVIEWED])
    ).all()

    count = 0

    for each in requests:
        entries = db.session.query(WFHRequestEntry).filter(
            WFHRequestEntry.request_id == each.request_id,
            WFHRequestEntry.status == Status.APPROVED,
            WFHRequestEntry.entry_date.in_(week_dates)
        ).all()
